...
class Morphology(MorphologyFST, Base):
    ...
    # word formation rules (i.e., strings of categories and delimiters)
    # separated by spaces -- system-generated value
    rules_generated = Column(UnicodeText)
    # word formation rules (i.e., strings of categories and delimiters)
    # separated by spaces -- user-generated value
    rules = Column(UnicodeText)
    # if True, the morphemes on the upper side of the tape are in <f|g|c>
    # form, else f format
    rich_upper = Column(Boolean, default=False)
    # if True, the morphemes on the lower side of the tape are in <f|g|c>
    # forma, else f format
    ...
    # NOTE: the include_unknowns attribute currently has no effect: it turned
    # out to be a major complication!
    ...
    def generate_rules_and_lexicon(self):
        try:
            ...
            LOGGER.warning('GOT EXCEPTION TRYING TO GENERATE RULES AND LEXICON')
            LOGGER.warning(e)
            return [], {}

    def _generate_rules_and_lexicon(self):
        """Generate morphotactic rules and a lexicon for this morphology based
        on its corpora.
        ...
        if (self.lexicon_corpus and
            ...
            for form in self.lexicon_corpus.forms:
                new_morphemes = self._extract_morphemes_from_form(
                    form, morpheme_splitter)
                for pos, data in new_morphemes:
                    morphemes.setdefault(pos, set()).add(data)
        # Get the pos sequences (and morphemes) from the user-specified
        # ``rules`` string value or else from the words in the rules corpus.
        pos_sequences = set()
        if self.rules:
    ...
    def _extract_morphemes_from_form(self, form, morpheme_splitter):
        """Return the morphemes in ``form`` as a list of tuples of the form
        (pos, (mb, mg)).
        """
    ...
    def _filter_invalid_sequences(self, pos_sequences, morphemes):
        """Remove category sequences from pos_sequences if they contain
        categories not listed as keys of the morphemes dict or if they contain
        delimiters not listed in self.delimiters.
        """
        if not morphemes:
    ...
    @staticmethod
    def generate_dictionary(lexicon):
        """Return a dictionary of lexical items, i.e., a mapping from morpheme
        forms to lists of gloss, category 2-tuples.

        :param dict lexicon: keys are categories and values are lists of
            2-tuples of the form <form, gloss>.

        This function need only be called if ``self.rich_upper`` is set to
        ``False``, in which case the morphology will parse, e.g., chien-s to
        chien-s and the dictionary will be needed to disambiguate the parse
        into richer representations, e.g., ['chien|dog|N-s|PL|Num',
        'chien|dog|N-s|plrl|Phi'].
        """
        dictionary = {}
    ...
    def write(self, unknown_category):
        ...
        with open(compiler_path, 'w') as f:
            if self.script_type == 'lexc':
                f.write('#!/bin/sh\nfoma -e "read lexc %s" -e "save stack %s"'
                        ' -e "quit"' % (script_path, binary_path))
            else:
    ...
    def get_morphology_generator(self, pos_sequences, morphemes):
        ...
        :param list pos_sequences: a sorte list of tuples containing sequences
            of categories and morpheme delimiters
        :param dict morphemes: keys are categories, values are lists of (form,
            gloss) 2-tuples
    ...
    def _get_lexc_morphology_generator(self, morphemes, pos_sequences):
        """Return a generator that yields lines of a foma script representing
        the morphology using the lexc formalism, cf.
        https://code.google.com/p/foma/wiki/MorphologicalAnalysisTutorial.

        :param dict morphemes: a dict from category names to lists of (form,
            gloss) tuples.
        :param list pos_sequences: a sorted list of tuples containing category
            names and morpheme delimiters.
        :yields: lines of a lexc foma script

        The OLD generates the names for lexc lexica from category (POS) plus
        delimiter sequences by joining the categories and delimiters into a
        string and using that string to generate an MD5 hash.  Thus the
        category sequence ('Asp', '-', 'V', '-', 'Agr') would imply a root
        category MD5('Asp-V-Agr') as well as the following continuation classes
        MD5('-V-Agr') MD5('V-Agr') MD5('-Agr') MD5('Agr')
        """

        # If not rich_upper, remove homographs from morphemes
        if not self.rich_upper:
            morphemes = dict(
                (pos, sorted(set((mb, None) for mb, mg in morph_list)))
    ...
    @staticmethod
    def _pos_sequence2lexicon_name(pos_sequence):
        """Return a foma lexc lexicon name for the tuple of categories and
        delimiters; output is an MD5 hash.
        """
    ...
    def _get_lexicon_entries_generator(self, pos_sequence, morphemes):
        """Return a generator that yields a line for each entry in a lexc
        LEXICON based on a POS sequence.

        :param tuple pos_sequence: something like ('N', '-', 'Ninf') or
            ('-', 'Ninf').
        :param dict morphemes: {'N': [(u'chien', u'dog'),
            (u'chat', u'cat'), ...], 'V': ...}
        :yields: lines that comprise the entries in a foma lexc LEXICON
            declaration.
        """
                    ...
                    if self.rich_upper and self.rich_lower:
                        yield u'%s %s;\n' % (
                            self.rare_delimiter.join(
                                [form, gloss, category_name]), next_class)
                    elif self.rich_upper:
                        yield u'%s:%s %s;\n' % (
                            self.rare_delimiter.join(
                                [form, gloss, category_name]), form, next_class)
                    else:
                        yield u'%s:%s %s;\n' % (
                            form, self.rare_delimiter.join(
                                [form, gloss, category_name]), next_class)
                else:
                    yield u'%s %s;\n' % (form, next_class)
        yield u'\n\n'
    ...
    def _get_regex_morphology_generator(self, morphemes, pos_sequences):
        """Return a generator that yields lines of a foma script representing
        the morphology using standard regular expressions.  Contrast this with
        the lexc approach utilized by ``get_lexc_morphology_generator``.

        :param dict morphemes: a dict from category names to lists of (form,
            gloss) tuples.
        :param list pos_sequences: a sorted list of tuples  whose elements are
            categories and morpheme delimiters.
    ...
    def _get_morpheme_representation(self, **kwargs):
        """Return a rich representation of the morpheme-as-FST (i.e.,
        <f|g|c>:f) or an impoverished one (i.e., f:f).
        """
        form = kwargs['mb']
        if self.rich_upper or self.rich_lower:
    ...
    def _get_lexicon_generator(self, morphemes):
        """Return a generator that yields lines of a foma script defining a
        lexicon.

        :param morphemes: dict from category names to lists of (mb, mg) tuples.
        :yields: unicode object (lines) that comprise a valid foma script
            defining a lexicon.

        .. note::

            The presence of a form of category N with a morpheme break value of
            'chien' and a morpheme gloss value of 'dog' will result in the
            regex defined as 'N' having 'c h i e n "|dog|N":0' as one of its
            disjuncts.  This is a transducer that maps 'chien|dog|N' to
            'chien', i.e,. '"|dog|N"' is a multi-character symbol that is
            mapped to the null symbol, i.e., '0'.  Note also that the vertical
            bar '|' character is not actually used -- the delimiter character
            is actually that defined in ``utils.rare_delimiter`` which, by
            default, is U+2980 'TRIPLE VERTICAL BAR DELIMITER'.
        """
        ...
        for pos, data in sorted(morphemes.items()):
            ...
            if foma_regex_name:
                ...
                if data:
                    ...
                    yield u'    %s \n' % self._get_morpheme_representation(
                        mb=data[-1][0], mg=data[-1][1], pos=pos,
                        delimiter=delimiter)
                yield u'];\n\n'

    def _get_valid_foma_regex_name(self, candidate):
        """Return the candidate foma regex name with all reserved symbols
        removed and suffixed by "Cat".  This prevents conflicts between regex
        names and symbols in regexes.
        """
    ...
    def _pos_sequence2foma_disjunct(self, pos_sequence):
        ...
        :returns: a unicode object representing a foma disjunct, e.g.,
            u'AGR "-" V'
        """
        tmp = []
    ...
    def _get_word_formation_rules_generator(self, pos_sequences):
        """Return a generator that yields lines for a foma script defining
        morphological rules.

        :param list pos_sequences: tuples containing categories and delimiters
        :yields: unicode objects (lines) that comprise a valid foma script
            defining morphological rules
        """
