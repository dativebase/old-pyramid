...
class Morphology(MorphologyFST, Base):
    """An OLD morphology is a finite-state transducer that encodes a set of
    lexical items (morphemes) and their possible orderings into grammatical
    words (morphotactics).
    """
    ...
    id = Column(
        Integer, Sequence('morphology_seq_id', optional=True), primary_key=True)
    UUID = Column(Unicode(36))
    name = Column(Unicode(255))
    description = Column(UnicodeText)
    script_type = Column(
        Unicode(5),
        doc='The type of FST script that the generated morphology should be'
        ' written in: one of “regex” or “lexc”.')
    lexicon_corpus_id = Column(
    ...
    lexicon_corpus = relation(
        ...
        doc='An OLD corpus from which the lexicon of an OLD morphology will be'
        ' extracted.')
    rules_corpus_id = Column(
        Integer, ForeignKey('corpus.id', ondelete='SET NULL'))
    rules_corpus = relation(
        'Corpus', primaryjoin='Morphology.rules_corpus_id==Corpus.id',
        doc='An OLD corpus from which the morphotactic rules of an OLD'
    ...
    extract_morphemes_from_rules_corpus = Column(
        ...
        doc='If extract morphemes from ruels corpus is set to true, then'
        ' morphemes for the OLD morphology will be extracted from the rules'
        ' corpus in addition to also being extracted from the lexicon corpus.')
    rules_generated = Column(
        UnicodeText,
        doc='The rules generated of an OLD morphology are the morphotactic'
        ' rules generated by an OLD during a morphology generate attempt.')
    rules = Column(
        UnicodeText,
        doc='The rules attribute of an OLD morphology is a user-specified'
        ' set of morphotactic rules, i.e., strings like “V-Agr”, i.e.,'
        ' sequences of categories and delimiters. This value is relevant if,'
        ' instead of having the system extract morphotactic rules from a'
        ' specified rules corpus, the user has chosen to specify the rules'
        ' manually.')
    rich_upper = Column(
        Boolean, default=False,
        doc='If rich upper is set to true, then an OLD morphology\'s script'
        ' value will represent morphemes on the upper side of the tape as (shape,'
        ' gloss, category) triples; if set to false (the default), then morphemes'
        ' on the upper side of the tape will be represented by their shapes only.')
    rich_lower = Column(
        Boolean, default=False,
        doc='If rich lower is set to true, then an OLD morphology\'s script'
        ' value will represent morphemes on the lower side of the tape as (shape,'
        ' gloss, category) triples; if set to false (the default), then morphemes'
        ' on the lower side of the tape will be represented by their shapes only.')
    include_unknowns = Column(
        Boolean, default=False,
        doc='If include unknowns is set to true, then morphemes of unknown'
        ' category will be added to the lexicon of the morphology. NOTE: the'
        ' include unknowns attribute currently has no effect on OLD'
        ' morphologies.')

    # These incidental attributes are initialized by the constructor.
    parent_directory = Column(
        Unicode(255),
        doc='The path to the directory on disk where an OLD morphology\'s'
        ' files are stored.')
    word_boundary_symbol = Column(
        Unicode(10),
        doc='The word boundary symbol of an OLD morphology is used to represent'
        ' the left and right edges of words.')
    rare_delimiter = Column(
        Unicode(10),
        doc='The rare delimiter of an OLD morphology is a seldom used Unicode'
        ' character that is used to separate the components (i.e., shape, gloss'
        ' and category) of morphemes.')
    morpheme_delimiters = Column(
        Unicode(255),
        doc='The morpheme delimiters of an OLD morphology are a set of'
        ' comma-separated delimiter characters that the morphology assumes are'
        ' used to separate morphemes in the analyzed words given as input to the'
        ' morphology.')
