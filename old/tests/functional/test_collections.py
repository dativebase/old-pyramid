...
class TestCollectionsView(TestView):
    ...
    def test_index(self):
        ...
        self.dbsession.add_all([restricted_tag, my_contributor])
        self.dbsession.commit()
        my_contributor = self.dbsession.query(old_models.User).filter(
        ...
        restricted_collection = self.dbsession.query(old_models.Collection).get(
            restricted_collection_id)
        restricted_collection.tags = []
        self.dbsession.add(restricted_collection)
        self.dbsession.commit()
        extra_environ = {'test.authentication.role': 'viewer'}
        response = self.app.get(url('index'), headers=self.json_headers,
                                extra_environ=extra_environ)
        ...
        self.dbsession.add_all(collections)
        self.dbsession.commit()
    ...
    def test_create(self):
        ...
        tag1.name = 'tag 1'
        tag2.name = 'tag 2'
        self.dbsession.add_all([tag1, tag2, restricted_tag])
        ...
        collection2 = self.dbsession.query(old_models.Collection).get(collection2_id)
        ...
        collection2_backups_count = self.dbsession.query(old_models.CollectionBackup).\
            filter(old_models.CollectionBackup.collection_id == collection2_id).count()
        collection3 = self.dbsession.query(old_models.Collection).get(collection3_id)
        ...
        collection3_backups_count = self.dbsession.query(old_models.CollectionBackup).\
        ...
        self.dbsession.expire(collection2)
        self.dbsession.expire(collection3)
        new_collection2 = self.dbsession.query(old_models.Collection).get(collection2_id)
        ...
        new_collection2_backups = self.dbsession.query(old_models.CollectionBackup).\
        ...
        new_collection3 = self.dbsession.query(old_models.Collection).get(collection3_id)
        ...
        new_collection3_backups = self.dbsession.query(old_models.CollectionBackup).\
        ...
        new_collection3_HTML = new_collection3.html
        new_collection2_backups_count = self.dbsession.query(old_models.CollectionBackup).\
            filter(old_models.CollectionBackup.collection_id == collection2_id).count()
        new_collection3_backups_count = self.dbsession.query(old_models.CollectionBackup).\
            filter(old_models.CollectionBackup.collection_id == collection3_id).count()
    ...
    def test_create_invalid(self):
        ...
        response = self.app.post(url('create'), params, self.json_headers,
                                self.extra_environ_admin, status=400)
        resp = response.json_body
        new_collection_count = self.dbsession.query(old_models.Collection).count()
        ...
        self.dbsession.add_all([speaker, source])
        self.dbsession.flush()
    ...
    def test_relational_restrictions(self):
        ...
        restricted_tag = omb.generate_restricted_tag()
        self.dbsession.add(restricted_tag)
        self.dbsession.flush()
        restricted_tag_id = restricted_tag.id
        self.dbsession.commit()
    ...
    def test_new(self):
        ...
        speaker = omb.generate_default_speaker()
        source = omb.generate_default_source()
        self.dbsession.add_all([application_settings, foreign_word_tag, restricted_tag,
                        speaker, source])
        self.dbsession.commit()
    ...
    def test_update(self):
        ...
        # succeed.
        orig_backup_count = self.dbsession.query(old_models.CollectionBackup).count()
        params = self.collection_create_params.copy()
        params.update({'title': 'Updated!'})
        params = json.dumps(params)
        response = self.app.put(url('update', id=id), params,
        ...
        resp = response.json_body
        new_collection_count = self.dbsession.query(old_models.Collection).count()
        new_backup_count = self.dbsession.query(old_models.CollectionBackup).count()
        ...
        collection_count_at_start = self.dbsession.query(old_models.Collection).count()
        ...
        tag1.name = 'tag 1'
        tag2.name = 'tag 2'
        self.dbsession.add_all([tag1, tag2])
        self.dbsession.flush()
        tag1_id = tag1.id
        tag2_id = tag2.id
        self.dbsession.commit()
    ...
    def test_delete(self):
        ...
        original_contributor_id = self.dbsession.query(old_models.User).filter(
        ...
        self.dbsession.add_all([application_settings, speaker, my_contributor, tag, file])
        self.dbsession.flush()
        tag_id = tag.id
        file_id = file.id
        speaker_id = speaker.id
        ...
        collection_count = self.dbsession.query(old_models.Collection).count()
        collection_backup_count = self.dbsession.query(old_models.CollectionBackup).count()
        ...
        new_collection_count = self.dbsession.query(old_models.Collection).count()
        new_collection_backup_count = self.dbsession.query(old_models.CollectionBackup).count()
        assert new_collection_count == collection_count + 1
        ...
        new_collection_count = self.dbsession.query(old_models.Collection).count()
        new_collection_backup_count = self.dbsession.query(old_models.CollectionBackup).count()
        tag_of_deleted_collection = self.dbsession.query(old_models.Tag).get(
            resp['tags'][0]['id'])
        file_of_deleted_collection = self.dbsession.query(old_models.File).get(
            resp['files'][0]['id'])
        speaker_of_deleted_collection = self.dbsession.query(old_models.Speaker).get(
            resp['speaker']['id'])
        ...
        deleted_collection = self.dbsession.query(old_models.Collection).get(to_delete_id)
        assert deleted_collection == None

        # The backed up collection should have the deleted collection's attributes
        backed_up_collection = self.dbsession.query(old_models.CollectionBackup).filter(
    ...
    def test_show(self):
        ...
        # able to view it too.
        restricted_collection = self.dbsession.query(old_models.Collection).get(restricted_collection_id)
        restricted_collection.tags = []
        self.dbsession.add(restricted_collection)
        self.dbsession.commit()
        extra_environ = {'test.authentication.role': 'viewer'}
    ...
    def test_edit(self):
        ...
        application_settings = omb.generate_default_application_settings()
        restricted_tag = omb.generate_restricted_tag()
        self.dbsession.add_all([restricted_tag, application_settings])
        self.dbsession.commit()
        restricted_tag = db.get_restricted_tag()
        ...
        speaker = omb.generate_default_speaker()
        source = omb.generate_default_source()
        self.dbsession.add_all([application_settings, foreign_word_tag, speaker, source])
        self.dbsession.commit()
    ...
    def test_new_search(self):
        ...
        query_builder = SQLAQueryBuilder(
            self.dbsession, 'Collection', settings=self.settings)
