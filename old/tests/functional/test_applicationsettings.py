...
class TestApplicationsettingsView(TestView):
    ...
    def test_index(self):
        ...
        with transaction.manager:
            ...
            application_settings = add_default_application_settings(dbsession)
    ...
    def test_create(self):
        ...
        with transaction.manager:
            ...
            params.update({
                ...
                'punctuation': '!?.,;:-_',
                'grammaticalities': '*,**,***,?,??,???,#,##,###',
                'unrestricted_users': [dbsession.query(User).filter(
    ...
    def test_update(self):
        ...
        with transaction.manager:
            ...
            application_settings_count = dbsession.query(
                ApplicationSettings).count()
            contributor_id = dbsession.query(User).filter(User.role=='contributor').first().id
            ...
            resp = response.json_body
            id = int(resp['id'])
            new_application_settings_count = dbsession.query(ApplicationSettings).count()
    ...
    def test_delete(self):
        ...
        with transaction.manager:
            ...
            # Add an orthography.
            orthography1 = omb.generate_default_orthography1()
            dbsession.add(orthography1)
            transaction.commit()
            orthography1 = db.get_orthographies()[0]
            orthography1_id = orthography1.id
            orthography1 = dbsession.query(Orthography).get(orthography1_id)
            ...
            response = self.app.post(url('create'), params,
                                     self.json_headers, self.extra_environ_admin)
            resp = response.json_body
            new_application_settings_count = dbsession.query(
                ApplicationSettings).count()
            ...
            deleted_application_settings = dbsession.query(
