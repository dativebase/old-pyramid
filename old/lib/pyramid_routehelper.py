...
def add_resource(self, handler, member_name, collection_name, **kwargs):
    ...
    def swap(dct, newdct):
        for key, value in dct.items():
            newdct.setdefault(value.upper(), []).append(key)
        return newdct
    ...
    def add_route_and_view(self, action, route_name, path, request_method='any'):
        ...
        print('pyramid_routehelper calling self.add_view with:')
        print('    handler')
        print('    ' + str(handler))
        print('    action')
        print('    ' + str(action))
        print('    route_name')
        print('    ' + str(route_name))
        print('    request_method')
        print('    ' + str(request_method))
        print('\n')

        self.add_view(view=handler, attr=action, route_name=route_name,
                      request_method=request_method,
                      **action_kwargs.get(action, {}).get('default', {}))
